...
public class MyEWrapperImpl implements EWrapper {
	...
	//! [socket_declare]
		...
		public void orderStatus(int orderId, String status, double filled,
			...
			//定单状态。可能的值包括：
			//PendingSubmit - 表示你已经传递了定单，但还未接到来自定单目的地接收的确认信息。 注：这个定单状态不是由TWS发送的，应该由API设计人设置为当定单发送时发出。
			//PendingCancel - 表示你已经发送了取消定单的请求，但还未收到来自定单目的地的取消确认。 此时，你的定单没有被确认取消。 在你的取消请求等待期间。你的定单仍有可能得到执行。 注：这个定单状态不是由TWS发送的，应该由API设计人设置为当定单发送时发出。
			//PreSubmitted - 表示模拟定单类型已经被IB系统接收，但还未被选中。定单被保持在IB系统中直到选择条件被满足。 届时，定单将被发送到指定的定单目的地。
